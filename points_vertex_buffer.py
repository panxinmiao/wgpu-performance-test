    ...
    vertex={
        ...
        "buffers": [
            ...
            },
            {
                ...
                "step_mode": "instance",
                "attributes": [ 
                    {"shader_location": 1, "offset": 0, "format": "float32"}, # size
                ],
            }
        ],
    },
...
# vertex_data = vertex_data*np.array([2, 2, 15]) - np.array([1, 1, -1])  # scale (x, y, size) to [-1, 1], [-1, 1], [1, 16]
...
positions_buffer = device.create_buffer(size=positions_data.nbytes, usage=wgpu.BufferUsage.VERTEX|wgpu.BufferUsage.COPY_DST)
...
sizes_buffer = device.create_buffer(size=sizes_data.nbytes, usage=wgpu.BufferUsage.VERTEX|wgpu.BufferUsage.COPY_DST)
...
def render():
    ...
    render_pass.set_vertex_buffer(0, positions_buffer)
    render_pass.set_vertex_buffer(1, sizes_buffer)
